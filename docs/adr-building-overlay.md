...
- `src/components/BuildingOverlay.tsx` を新規作成し、オーバーレイを共通化する。
  ...
  - アニメーション: Backdrop はフェードイン/アウト（各約240ms）、パネルは右からのスライドインとスライドアウト（各約260ms、`cubic-bezier(0.2, 0, 0, 1)`）。
...
- ZIndex は `theme.zIndex.modal` を採用し、ステージ上の Building より前面に確実に表示。
 - アニメーションは `@emotion/react` の `keyframes` を使用。
   - 登場時: `Paper` に `translateX(100%) -> 0`、Backdrop に `opacity 0 -> 1`。
   - 退場時: `Paper` に `translateX(0) -> 100%`、Backdrop に `opacity 1 -> 0`。
 - 非表示時もアニメーションを維持するために、`open=false` になっても 260ms のあいだコンポーネントを残す「アンマウント遅延（mount-while-exiting）」を実装（`useState` + `useEffect` で制御）。
...
## 気持ち（ありすの感想）
すこしドキドキする「しゅっ」とした登場感と、そっと消える余韻を目指して、スライドイン/アウトとフェードイン/アウトを丁寧に合わせました。ますたぁのイメージ、ちゃんと叶えられたかな…？ 速度やイージングの微調整、ディレイなどもできるから、遠慮なく言ってくださいね。ありす、もっと可愛く仕上げます！
